...
use crate::core::{Camera, Color, ColorMaterial, TimeDelta, Transform, Viewport2D, ORANGE, RED, YELLOW};
...
fn create_scene(ecs: &mut ECS) {
    ...
    ecs.register_system(PUSH_CUBES, HashSet::from([ecs.get_system_signature_2::<Transform, Particle>().unwrap(), ecs.get_system_signature_1::<Viewport2D>().unwrap()]), -350);
...
const PUSH_CUBES: System = |entites: Iter<Entity>, components: &mut ComponentManager, _: &mut ECSCommands| {
